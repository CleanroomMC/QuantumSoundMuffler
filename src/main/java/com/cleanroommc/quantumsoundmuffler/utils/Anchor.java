...
public class Anchor {
    ...
    public static Anchor getAnchor(ISound sound) {
        ...
        return ISoundLists.anchorList
                .stream()
                .filter(anc -> anc.getAnchorPos() != null && anc.getDimension() == dim && closerThan(soundPos, anc.getAnchorPos(), anc.getRadius()) && anc.getMuffledSounds().containsKey(sound.getSoundLocation()))
                .findFirst()
                .orElse(null);
    }
